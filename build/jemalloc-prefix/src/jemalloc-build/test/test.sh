...
for t in $@; do
  ...
  echo "=== ${t} ==="
  if [ -e "/root/share/fluent-bit/lib/jemalloc/${t}.sh" ] ; then
    ...
    $(enable_fill=1 \
      enable_prof=0 \
      enable_tcache=1 \
      . /root/share/fluent-bit/lib/jemalloc/${t}.sh && \
      export_malloc_conf && \
      ${t} /root/share/fluent-bit/lib/jemalloc/ /root/share/fluent-bit/build/jemalloc-prefix/src/jemalloc-build/)
  else
    $(export MALLOC_CONF= && \
      export_malloc_conf &&
      ${t} /root/share/fluent-bit/lib/jemalloc/ /root/share/fluent-bit/build/jemalloc-prefix/src/jemalloc-build/)
