...
void flb_tail_file_remove(struct flb_tail_file *file)
{
	...
	if (file->config->db) {
		printf("fluent-bit exit and set offset,file name: %s\n",file->name);
...
int flb_tail_file_chunk(struct flb_tail_file *file)
{
    ...
    if (bytes > 0) {
		...
		//当count达到一定次数后，将offset写入到db
        //if (file->config->db&&++count==file->config->db_count) {
        //    flb_tail_db_file_offset(file, file->config);
		//	count = 0;
        //}
